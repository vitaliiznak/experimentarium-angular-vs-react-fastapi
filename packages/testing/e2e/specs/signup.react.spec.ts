...
test.describe('React Signup Flow', () => {
  const TEST_USER = {
    name: 'Test User',  // React signup form includes name field
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('http://localhost:5173/signup');  // React dev server port
    await page.waitForLoadState('networkidle');
  });

  test('successful signup', async ({ page }) => {
    // Fill in signup form
    await page.getByTestId(TEST_IDS.SIGNUP_NAME).fill(TEST_USER.name);
  ...
  test('validation errors', async ({ page }) => {
    ...
    await page.getByTestId(TEST_IDS.SIGNUP_SUBMIT).click();
    await expect(page.getByTestId(TEST_IDS.SIGNUP_ERROR)).toContainText('Passwords do not match');
  ...
  test('existing email error', async ({ page }) => {
    ...
    await expect(page.getByTestId(TEST_IDS.SIGNUP_ERROR)).toBeVisible();
    await expect(page.getByTestId(TEST_IDS.SIGNUP_ERROR)).toContainText('already registered');
  });
}); 